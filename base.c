...
DB* db_init(Person *persons_array, int num_elements) {
    ...
    unsigned long array_start = (unsigned long) malloc(num_elements * sizeof(Person)+ 32);
    // 2.K : Décaler le début du tableau pour forcer l'alignement
    array_start = array_start - array_start %32 +32;
    ...
    int offset = address_first % 64;
/*     if (offset !=32 && offset != 0 && offset != 64){
        address_first = address_first + address_first %32;
    }; */

    printf("address_first: %lu offset: %d \n", address_first, offset);
    // can change address here
    int offset2 = address_second % 64;
    printf("address_second: %lu offset: %d \n", address_first, offset2);
    // can change address here
